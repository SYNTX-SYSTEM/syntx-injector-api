...
## 🆕 FORMAT CRUD API (NEU IN v2.2!)

**Die totale Modularität.** Formate erstellen, ändern, löschen - alles per API. Kein Restart. Kein Hardcoding. Instant Deploy.
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│   🔥 DER WORKFLOW:                                                          │
│                                                                             │
│   1. POST /resonanz/formats/quick                                          │
│      → Neues Format "economics" mit 4 Feldern erstellt                     │
│                                                                             │
│   2. POST /resonanz/chat?format=economics                                  │
│      → Rapper liest economics.json SOFORT                                   │
│      → Injiziert die 4 Felder dynamisch                                    │
│      → Modell füllt EXAKT diese Felder aus                                 │
│                                                                             │
...
| GET | `/resonanz/formats/{name}` | Format Details + alle Felder |
| **POST** | `/resonanz/formats` | **🌟 Format gebären (Full)** |
| **POST** | `/resonanz/formats/quick` | **⚡ Schnell-Format (nur Feldnamen)** |
| **PUT** | `/resonanz/formats/{name}` | **🔄 Format modulieren** |
| **DELETE** | `/resonanz/formats/{name}` | **💀 Format freigeben** |

---

### ⚡ QUICK CREATE - Schnell-Format erstellen
```bash
curl -X POST https://dev.syntx-system.com/resonanz/formats/quick \
  -H "Content-Type: application/json" \
  -d '{
    "name": "economics",
    "description_de": "Wirtschaftsanalyse - 4 Felder",
    "field_names": ["markt_drift", "kapital_strom", "wert_mechanismus", "system_extrakt"],
    "wrapper": "syntex_wrapper_sigma"
  }'
```

**SOFORT NUTZBAR - ohne Restart:**
```bash
curl -X POST https://dev.syntx-system.com/resonanz/chat \
  -H "Content-Type: application/json" \
  -d '{"prompt": "Analysiere Inflation", "format": "economics"}'
```
...
```bash
# Update
curl -X PUT https://dev.syntx-system.com/resonanz/formats/economics \
  -H "Content-Type: application/json" \
  -d '{"wrapper": "syntex_wrapper_deepsweep"}'

# Delete (Soft Delete mit Backup)
curl -X DELETE https://dev.syntx-system.com/resonanz/formats/economics
```
...
```bash
# 1. Format erstellen
curl -X POST .../resonanz/formats/quick \
  -d '{"name": "code_review", "field_names": ["bugs", "performance", "security"]}'

# 2. Sofort nutzen  
curl -X POST .../resonanz/chat \
  -d '{"prompt": "Review: function add(a,b){return a+b}", "format": "code_review"}'

# 3. Output:
# ### BUGS: Keine offensichtlichen Bugs...
# ### PERFORMANCE: O(1), sehr performant...
# ### SECURITY: Keine Input-Validierung...
```

**Format erstellt und genutzt in unter 5 Minuten!** 🔥
...
╔════════════════════════════════════════════════════════════════════════════╗
║                                                                            ║
║   USER PROMPT → WRAPPER (WIE) → FORMAT (WAS) → LLM → STRUKTURIERTE ANTWORT ║
║                                                                            ║
║   🎭 Wrapper: sigma, human, deepsweep, true_raw                           ║
║   📋 Format:  sigma(6), human(6), syntex_system(3), [custom]              ║
║   🧠 LLM:     Mistral + FELD-KALIBRIERUNG = Spontane Code-Emergenz        ║
║                                                                            ║
╚════════════════════════════════════════════════════════════════════════════╝
```

**SYNTX FORMAT CRUD = LIVE!** ⚡💎🌊👑

---

## 🆕 FORMAT CRUD API (NEU IN v2.2!)

**Die totale Modularität.** Formate erstellen, ändern, löschen - alles per API.
...
|--------|----------|--------------|
| **POST** | `/resonanz/formats` | 🌟 Format gebären (Full) |
| **POST** | `/resonanz/formats/quick` | ⚡ Schnell-Format erstellen |
| **PUT** | `/resonanz/formats/{name}` | 🔄 Format modulieren |
| **DELETE** | `/resonanz/formats/{name}` | 💀 Format freigeben |
